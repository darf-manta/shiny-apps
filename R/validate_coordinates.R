...
validate_coordinates = function(coordinate1, coordinate2) {
    ...
    min_east = 166021.44
    max_east = 833978.56
    min_north = ifelse(UTM_EPSG > 32700, 1116915.04, 0.00)
    max_north = ifelse(UTM_EPSG > 32700, 10000000.00, 9329005.18)

    # definir limites geográficos válidos
    min_latitude = ifelse(UTM_EPSG > 32700, -80.00, 0.00)
    max_latitude = ifelse(UTM_EPSG > 32700, 0.00, 84.00)
    ...
    if(dplyr::between(coordinate1, min_latitude, max_latitude)) {
        if(dplyr::between(coordinate2, min_longitude, max_longitude)) {
            coordinates = c(coordinate2, coordinate1)
            epsg = 4326
        } else error = "La coordenada geográfica ingresada no es válida."
    }
    if(dplyr::between(coordinate2, min_latitude, max_latitude)) {
        if(dplyr::between(coordinate1, min_longitude, max_longitude)) {
    ...
    if(dplyr::between(coordinate1, min_east, max_east)) {
        if(dplyr::between(coordinate2, min_north, max_north)) {
            coordinates = c(coordinate1, coordinate2)
            epsg = UTM_EPSG
        } else error = "La coordenada UTM ingresada no es válida."
    }
    if(dplyr::between(coordinate2, min_east, max_east)) {
        if(dplyr::between(coordinate1, min_north, max_north)) {
            coordinates = c(coordinate2, coordinate1)
            epsg = UTM_EPSG
        } else error = "La coordenada UTM ingresada no es válida."
    }
